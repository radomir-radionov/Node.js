...
exports.validateConfig = (config) => {
  ...
  if ('beforeMethods' in config) {
    if (
      typeof config.beforeMethods !== 'object'
      || Array.isArray(config.beforeMethods)
    ) {
    ...
    Object.keys(config.beforeMethods).forEach((before) => {
      if (!(before in config.methods)) {
        this.throwRpcErr(
          `JSON-RPC error: beforeMethod should have the same name as method, passed: ${before}`
        )
      }
    })
  }
  if ('afterMethods' in config) {
    if (
      typeof config.afterMethods !== 'object'
      || Array.isArray(config.afterMethods)
    ) {
    ...
    Object.keys(config.afterMethods).forEach((after) => {
      if (!(after in config.methods)) {
        this.throwRpcErr(
          `JSON-RPC error: afterMethods should have the same name as method, passed: ${after}`
        )
      }
    })
  }
...
exports.executeHook = (hook, params, result, raw) => {
  ...
  if (Array.isArray(hook)) {
    return Promise.all(hook.map((h) => h(params, result, raw)))
  }
